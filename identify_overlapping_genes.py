...
class overlapping_genes():
    ...
    def __init__(self):
        ...
        self.folder = '/home/aled/Documents/161125_coverage_reports/'
        # input bedfile
        self.bed_file = self.folder + "161123_NRASdemo_AJdata.bed"
        # input refseq file
        self.refseq_file = self.folder + "161123_NRASdemo_AJdataRefSeqFormat.txt"
        ...
        for i in set(df['chr']):
            ...
            for index, row in a.iterrows():
                ...
                if stop == 0:
                    ...
                    if row['start'] <= stop:
                        if index.rstrip() in self.overlap_genes:
                            pass
                        else:
                            if self.bedfileB:
                                self.overlap_genes.append(prev_gene)
    ...
    def check_bedfileB(self):
        bedfileB = open(self.folder + self.panname + "B.bed", 'r')
        self.dict.clear()
        self.gene_start_stop_dict.clear()
        del self.overlap_genes[:]

        # ignore header
        for line in bedfileB:
            if line.startswith("#"):
                pass
            else:
                # split
                ...
                if GeneSymbol in self.dict:
                    ...
                    self.dict[GeneSymbol] = [(chr, start, stop)]
        self.bedfileB=True
        self.define_gene_boundaries()
        self.identify_overlaps()
        if len(self.overlap_genes)>0:
            print "warning overlapping genes in bedfile B :"+ str(self.overlap_genes)
